...
function addGamesToPage(games) {
    ...
    for (let game of games) {
        ...
        gameCard.innerHTML = `<img class="game-img" src="${game.img}"/<br><h2>${game.name}</h2><br><p>Pledged: ${game.pledged}</p><br><p>${game.description}</p>`;
...
function filterUnfundedOnly() {
    ...
    const gamesNotMetGoal = GAMES_JSON.filter( (game) => {
...
const gamesUnfundedCount = GAMES_JSON.reduce( (total, game) => {
    return game.pledged < game.goal ? total + 1 : total;
}, 0);


// create a string that explains the number of unfunded games using the ternary operator
const displayStr = `A total of $${amountRaised.toLocaleString()} has been raised for ${totalGames} game${totalGames != 1 ? `s` : ``}. Currently, ${gamesUnfundedCount} game${gamesUnfundedCount!=1 ? `s`: ``} remain unfunded.`
...
const [firstGame, secondGame, ...others] = GAMES_JSON;
